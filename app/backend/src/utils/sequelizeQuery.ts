...
export const homeAwayLeaderboard = `SELECT 
* 
FROM (SELECT 
home.name,
...
SELECT 
  *, 
  FORMAT((classificação.totalPoints) / (classificação.totalGames * 3) * 100, 2)
      as "efficiency"
FROM (SELECT 
    team_name as "name",
    (SUM(case 
    ...
    COUNT(mt.away_team) as "totalGames",
    SUM(case when mt.away_team_goals > mt.home_team_goals then 1 else 0 end)
          as "totalVictories",
    SUM(case when mt.away_team_goals = mt.home_team_goals then 1 else 0 end)
          as "totalDraws",
    SUM(mt.away_team_goals) as "goalsFavor",
